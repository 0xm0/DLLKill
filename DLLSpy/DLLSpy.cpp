...
ofstream fLogFile;
ofstream fNewLog;
string sSystemPaths[] = { "C:\\Windows\\System32", "C:\\Windows\\System", "C:\\Windows" }; // change the directory you want checked here, for example F:\\Windows
...
int _tmain(int argc, TCHAR* argv[])
{
	...
	string banner =
		R"( ______   _        _        _       _________ _        _       
(  __  \ ( \      ( \      | \    /\\__   __/( \      ( \      
| (  \  )| (      | (      |  \  / /   ) (   | (      | (      
| |   ) || |      | |      |  (_/ /    | |   | |      | |      
| |   | || |      | |      |   _ (     | |   | |      | |      
| |   ) || |      | |      |  ( \ \    | |   | |      | |      
| (__/  )| (____/\| (____/\|  /  \ \___) (___| (____/\| (____/\
(______/ (_______/(_______/|_/    \/\_______/(_______/(_______/
                                                               

Advacancements By 0xm0             
)";
	cout << banner << endl;

	if (!IsUserAnAdmin())
	{
		cout << "DLLKill must be activated with elevated privileges, shutting down.." << endl;
		goto lblCleanup;
	}
...
lblCleanup:
	if (ESTATUS_FAILED(eReturn) && eReturn != ESTATS_MISSING_ARGUMENTS)
		cout << "DLLKill exited with error: " << eReturn << endl;
	return 0;
}
...
lblCleanup:
	if (eReturn == ESTATS_MISSING_ARGUMENTS)
	{
		...
		cout << "-X ['/specfic/path'] [high/medium/low] Find and kill all bad DLLs immediatly.   " << endl;
		cout << "                                       Please note you should only run this if you are comfortable and have no worries if it breaks any applications." << endl;
		cout << "                                       This can damage lots of software or applications it thinks is dangerous/medium or low." << endl;
		cout << "               Please note: This does hunt any bad applications and deletes the root of their cause, ensuring you have not been a DLL Ratted or Hijacked enitely." << endl;
